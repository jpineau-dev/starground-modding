...
@export var canBreakWithAttack = false
...
@rpc("call_local")
func on_hit(changeDrop: bool = true, hitStrength: float = 1, playerHit: bool = false) -> void :
...
func spawn_items():
    ...
    if dropItems:
        for i in range(dropData.size()):
            ...
            Global.main.create_item(dropItem, position, 50)
